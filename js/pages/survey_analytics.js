// ============================================================================
// アンケート分析ページ (js/pages/survey_analytics.js)
// js/pages/form_analytics.js (既存の分析フォーム 分析画面) を複製・改修したもの。
// UI/集計ロジックはほぼ同一だが、以下の点のみ異なる:
//   1. 単一の meo_form ではなく、surveyId ごとに複数のアンケートを分析できる
//      (Store.getSurvey/getSurveyStats/getSurveyResponses/clearSurveyStatsAndResponses)。
//   2. 分析フォーム専用の「今月◯名が診断を申込しています」表示ロジック
//      (FIXED_MONTHLY_SEQUENCES等)はアンケート機能には関係がないため含めていない。
// ============================================================================

App.Pages.survey_analytics = async function(surveyId) {
    ...
    App.mount(`
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px;">
            <h2 style="margin:0; font-size:1.5rem; font-weight:700;"><i class="ph ph-chart-bar" style="margin-right:8px; color:var(--primary-color);"></i>アンケート分析</h2>
            <div>
                ...
                <button class="btn btn-secondary" id="open-form-btn" style="margin-right:8px;"><i class="ph ph-arrow-square-out"></i> フォームを開く</button>
                <button class="btn btn-primary" id="edit-form-btn"><i class="ph ph-pencil-simple"></i> アンケートを編集</button>
            </div>
        </div>
    ...
    if (openBtn) {
        openBtn.addEventListener('click', () => window.open(`/surveyform/index.html?id=${surveyId}`, '_blank'));
    }
    const editBtn = document.getElementById('edit-form-btn');
    if (editBtn) {
        editBtn.addEventListener('click', () => App.navigate('survey_editor', surveyId));
    }
    ...
    setTimeout(() => {
        ...
        if (resetBtn) {
            resetBtn.addEventListener('click', async () => {
                if (confirm('【警告】このアンケートのテスト回答データやアクセス統計ログ（Views, Starts等）がすべて削除されます。よろしいですか？')) {
                    resetBtn.innerHTML = '<i class="ph ph-spinner ph-spin"></i> リセット中...';
                    resetBtn.disabled = true;
                    try {
                        await Store.clearSurveyStatsAndResponses(surveyId);
                        alert('データをすべてリセットしました。');
                        App.Pages.survey_analytics(surveyId);
                    } catch (e) {
                        console.error(e);
    ...
    const [formData, statsData, responsesData] = await Promise.all([
        Store.getSurvey(surveyId),
        Store.getSurveyStats(surveyId),
        Store.getSurveyResponses(surveyId)
    ]);

    if (!formData) {
        document.getElementById('analytics-content').innerHTML = `
            <div class="card" style="padding:40px;">
                <h3 style="margin-bottom:16px;">アンケートが見つかりません</h3>
                <button class="btn btn-primary" onclick="App.navigate('surveys')"><i class="ph ph-list"></i> 一覧へ戻る</button>
            </div>
        `;
        return;
    }
    ...
    document.getElementById('analytics-content').innerHTML = `
        <div style="text-align:left;">
            <div class="card" style="padding:16px 24px; margin-bottom:24px; background:#f8f9fc;">
                <div style="font-size:0.85rem; color:var(--text-secondary); font-weight:600;">アンケート名</div>
                <div style="font-size:1.2rem; font-weight:700; margin-top:4px;">${formData.title || '無題のアンケート'}</div>
            </div>
            ${kpiHtml}
            ${funnelHtml}
            ${responsesHtml}
    ...
    if (downloadCsvBtn) {
        downloadCsvBtn.addEventListener('click', () => {
            ...
            link.setAttribute("download", `survey_${surveyId}_responses_${new Date().toISOString().split('T')[0]}.csv`);
