// ============================================================================
// アンケート編集ページ (js/pages/survey_editor.js)
// js/pages/form_editor.js (既存の分析フォーム エディタ) を複製・改修したもの。
// UI/操作感は分析フォームエディタと同一だが、以下の点のみ意図的に異なる:
//
//   1. 単一の meo_form ではなく、surveyId ごとに複数のアンケートを編集できる
//      (Store.getSurvey(surveyId) / Store.saveSurvey(surveyId, formData) を使用)。
//
//   2. 質問タイプから「カレンダー日程調整 (calendar_booking)」を除外し、
//      「🔌 外部連携設定 (UTAGE/カレンダー/Chatwork)」パネルも表示しない。
//      理由: api/calendar.js・api/booking.js が service_type='meo_form' 固定で
//      フォームを検索する実装になっており、このままではアンケート
//      (service_type='survey_definition') に対しては動作しない。かつ、これらは
//      本番稼働中のAPIでありこの環境から実際に呼び出してテストする手段がないため、
//      未検証のままバックエンドを改修して既存の分析フォームの日程調整機能を壊す
//      リスクを避け、v1では意図的にスコープ外とした(詳細はjs/state.js内コメント参照)。
//      ご要望があれば、api/*.js側を改修のうえ追加対応することも可能。
//
//   3. Chatwork通知自体は、送信時(surveyform/form.js側)で既存のapi/chatwork.jsを
//      そのまま再利用して自動送信する(通知先は分析フォームと共通の1部屋)。
//      ただし、認証情報(APIトークン/ルームID)を上書きする「連携設定を保存」ボタンは
//      このページには置かない(誤って分析フォーム側のUTAGE/カレンダー設定まで
//      巻き込んで上書きしてしまう事故を防ぐため)。
// ============================================================================

App.Pages.survey_editor = async function(surveyId) {
    ...
    let formData = await Store.getSurvey(surveyId);
    if (!formData) {
        App.mount(`
            <div class="card" style="margin-top:24px; padding: 40px; text-align:center;">
                <h3 class="card-title">アンケートが見つかりません</h3>
                <button class="btn btn-primary" style="margin-top:16px;" id="back-to-surveys-btn">アンケート一覧へ戻る</button>
            </div>
        `, () => {
            const btn = document.getElementById('back-to-surveys-btn');
            if (btn) btn.addEventListener('click', () => App.navigate('surveys'));
        });
        return;
    }

    if (!formData.theme) formData.theme = { titleColor: "#1a1a1a", descColor: "#666666", titleSize: "1.5rem", descSize: "1rem" };
    if (!formData.review) formData.review = { title: "回答内容の確認", description: "以下の内容でよろしいですか？", buttonText: "この内容で提出する", align: "left", descAlign: "left" };
    ...
    const render = () => {
        ...
        const html = `
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px;">
                <h2 style="margin:0; font-size:1.5rem; font-weight:700;"><i class="ph ph-pencil-simple" style="margin-right:8px; color:var(--primary-color);"></i>アンケート エディタ</h2>
                <div>
                    <input type="text" id="form-main-title" class="input-field" style="width:300px; display:inline-block; margin-right:16px; margin-bottom:0;" value="${formData.title}" placeholder="アンケートの全体タイトル">
                    <button class="btn btn-secondary" style="margin-right:8px;" onclick="App.navigate('surveys')"><i class="ph ph-list"></i> 一覧へ戻る</button>
                    <button class="btn btn-secondary" style="margin-right:8px;" onclick="App.navigate('survey_analytics', '${surveyId}')"><i class="ph ph-chart-bar"></i> 分析へ</button>
                    <button class="btn btn-primary" id="save-form-btn"><i class="ph ph-floppy-disk"></i> アンケートを保存</button>
                </div>
            </div>
        ...
        App.mount(html, () => {
            ...
            if (saveBtn) {
                saveBtn.addEventListener('click', async () => {
                    ...
                    await Store.saveSurvey(surveyId, formData);
                    saveBtn.innerHTML = '<i class="ph ph-check"></i> 保存完了';
                    setTimeout(() => { saveBtn.innerHTML = '<i class="ph ph-floppy-disk"></i> アンケートを保存'; }, 2000);
                });
            }
        ...
        html += `<hr style="margin:32px 0; border:none; border-top:1px solid #e0e0e0;">
            ...
            <div class="form-group">
                ...
                <div style="display:flex; gap:8px;">
                    <input type="text" id="tracking-source-name" class="input-field" placeholder="経路名を入力...（空欄でも発行できます）" style="flex:1;">
    ...
    const bindSettingsEvents = (obj, type) => {
        ...
        // トラッキングURL生成ハンドラ (surveyform/index.html?id=surveyId&ref=...)
        ...
        if (generateUrlBtn) {
            generateUrlBtn.addEventListener('click', () => {
                ...
                const baseUrl = window.location.href.replace('/index.html', '/surveyform/index.html');
                const url = new URL(baseUrl);
                url.searchParams.set('id', surveyId);
                if (sourceName) url.searchParams.set('ref', sourceName);
